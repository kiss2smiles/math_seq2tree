...
def transfer_num(data):  # transfer num into "NUM"
    ...
    # \d+\.\d+%?:        匹配一个百分号小数或者一个小数，eg: 6.66、6.66%
    # \d+%?:             匹配一个百分数或者一个整数，eg: 95%、95
    ...
    for d in data:
        ...
        # COMMENT
        # segmented_text: 镇海 雅乐 学校 二年级 的 小朋友 到 一条 小路 的 一边 植树 ． 小朋友 们 每隔 2 米 种 一棵树
        #                （ 马路 两头 都 种 了 树 ） ， 最后 发现 一共 种 了 11 棵 ， 这 条 小路 长 多少 米 ．
        # input_seq =  ['镇海', '雅乐', '学校', '二年级', '的', '小朋友', '到', '一条', '小路', '的', '一边', '植树', '．',
        #               '小朋友', '们', '每隔', 'NUM', '米', '种', '一棵树', '（', '马路', '两头', '都', '种', '了', '树', '）',
        #               '，', '最后', '发现', '一共', '种', '了', 'NUM', '棵', '，', '这', '条', '小路', '长', '多少', '米', '．']
        # equation: x=(11-1)*2
        # nums: 文本中出现的数字, eg, [2, 11]

        # nums_fraction: 文本中的分数
        nums_fraction = []
        ...
        # generate_nums: 文本中未出现的常数，eg: ['1']
        # generate_nums_dict: 文本中未出现的常数与索引的映射，eg: {'1': 1}
...
def prepare_data(pairs_trained, pairs_tested, trim_min_count, generate_nums, copy_nums, tree=False):
    ...
    # input_lang:  input vocab
    # output_lang: output vocab
    #
    # train_pairs:
    #     pair[0] = input_seq: ['时代', '超市', '“', 'NUM', '一', '”', '大', '酬宾', '，', '全场', '家电', '按', 'NUM', '销售',
    #                           '，', '原价', 'NUM', '元', '的', '电饭锅', '，', '现在', '售价', '=', '多少', '元', '．']
    #     pair[1] = out_seq:   ['*', 'N2', 'N1']
    #     pair[2] = nums:      ['5', '0.8', '150']
    #     pair[3] = num_pos:   [3, 12, 16]
    #     input_word_idxs
...
def prepare_train_batch(pairs_to_batch, batch_size):
    ...
    # num_stack_batches: []
    # num_pos_batches:   [18, 25, 32] = [N1, N2, N3]
    # num_size_batches:  3 = len(num_pos)
    return input_batches, input_lengths, output_batches, output_lengths, \
           nums_batches, num_stack_batches, num_pos_batches, num_size_batches
